...
try {
    ...
    $sql = "SELECT df.item_id, df.condemnation_reason, r.name_of_the_item, r.lab_id 
    ...
    $sql = "INSERT INTO past_disposals (item_id, item_name, disposal_date, reason, lab_id) 
            VALUES (?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("issss", $item['item_id'], $item['name_of_the_item'], $disposal_date, $item['condemnation_reason'], $item['lab_id']);
    if (!$stmt->execute()) {
